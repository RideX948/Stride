...
export async function getSessionToken(): Promise<string | null> {
  try {
    // Web: the server runs on a different port than Metro, so the session
    // cookie is cross-origin (SameSite=Lax) and won't be sent on API calls.
    // Persist the token in localStorage and send it as a Bearer header instead.
    if (Platform.OS === "web") {
